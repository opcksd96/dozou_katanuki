### 第5章：プレゼンテーション層概論（Foolish Frontend & 宣言型UI）
**プロジェクト名** : dozou_katanuki (Pluggable UI & Multi-Format Local Archival System "土蔵・型抜き") Pluggable UI & Multi-Format Archival System)  
**ドキュメントID** : SPEC-PRESENTATION-001  
**バージョン** : 3.6.0  
**作成日** : 2026-08-17  
**ステータス** : 正式仕様（Dumb UI一般化・シグナル・UDF原則、設定管理系コンポーネント、SVGプレースホルダーフィラー出し分け・3段階メディアステータス表示、外部プラグイン統合規約、汎用ルーター）  

**Navigation** : [← 前の章: 第4章：データベース設計と仮想ストレージプール](04_database_and_virtual_storage_pool.md) | [📚 目次 (Home)](README.md) | [次の章: 第6章：ミドルウェア層（Middleware Hub & Proxy） →](06_thick_middleware_and_proxy_v4.md)

--------------------------------------------------------------------------------

#### 5.1 宣言型UIと単一データフロー（UDF）パラダイム
...
```
                       [ ユーザー操作 (Event / Action) ]
                                     ...
                                     ▼
 ┌────────────────────────────────────────────────────────────────────────┐
 │ 1. Stateless Pure View (components/*.vue)                              │
 │  - Dumb UI, 宣言型レンダリング, Props（RenderTree）を受け取るだけ       │
 └───────────────────▲─────────────────────────────┬──────────────────────┘
                     │ (UDF: 一方向データフェッチ)   │ (Event / Action 発火)
                     │                             ▼
 ┌───────────────────┴────────────────────────────────────────────────────┐
 │ 2. State & Signal Composable (composables/*.ts)                        │
 │  - Vue 3 ref/reactive (シグナルベース状態管理)                          │
 │  - 外部 API (:5175) からデータを取得, View へ一方向（Top-to-Bottom）配信│
 └────────────────────────────────────────────────────────────────────────┘
```

---

#### 5.2 プラットフォーム共通 URL ルーティング体系（Vue Router / History API） [38]
...
#### 5.3 主要フロントエンド型定義 (RenderTree / RenderMedia Status)
...
#### 5.4 Skin Controller 共通インターフェース規格 [100]
...
#### 5.7 フロントエンド専用外部プラグイン ＆ 共通コアライブラリ仕様
