...
- **メタデータ**: SQLite3 mediaテーブルに以下を保存
  ...
  - `processing_state`: 'queued' | 'completed' | 'failed' | 'archived'
  - `width`, `height`: ピクセル単位
...
#### 3.1.1 メタデータ更新
- **対象**: mediaテーブルの `processing_state`, `is_sensitive`, `blurhash`, `dominant_color`
...
AFTER UPDATE OF processing_state ON media
FOR EACH ROW
...
| `UPDATE media SET processing_state = 'failed'` | 許可 | 再試行ロジックへ誘導 |
| `UPDATE media SET processing_state = 'archived'` | 許可 | 論理アーカイブ（データは保持） |
...
BEGIN
    UPDATE media SET processing_state = 'archived' WHERE tweet_id = NEW.id;
END;
```
...
|-----------|----------------|------------------------|------------------|
| `active` | 表示 | 表示 | 可 |
| `in_progress` | 非表示 | 進行インジケータ表示 | 可 (一時的) |
| `completed` | 表示 | 表示 | 可 |
...
| ステータス | 英語表記 | 説明 | 表示フラグ |
|-----------|----------|------|-----------|
| `queued` | Queued | ストレージへの保存待ち | 不明 |
| `in_progress` | In Progress | ダウンロード・トランスコード実行中 | 進行中インジケータ |
| `completed` | Completed | 正常に保存完了 | 通常表示 |
...
stateDiagram-v2
    [*] --> queued: メディア取得開始
    queued --> in_progress: ダウンロード開始
    in_progress --> completed: 保存成功
    in_progress --> failed: ネットワークエラー or ストレージ容量超過
    failed --> queued: リトライ実行 (最大3回)
    failed --> archived: 論理アーカイブ (失敗データも永久保存)
    completed --> archived: ツイートアーカイブ時
    archived --> completed: 管理者による復元リクエスト
...
| トリガー条件 | 発生元 | 遷移先 | 説明 |
|-------------|--------|--------|------|
| ダウンロード開始 | メディアフェッチ開始 | `queued → in_progress` | ARIA2 または JD2プロセス起動 |
| 保存成功 | Stashapp GraphQLレスポンス200 | `in_progress → completed` | メタデータDB登録完了 |
| ネットワークエラー (1回きれ) | ダウンロード失敗 | `in_progress → failed` | リトライカウント残り |
| ネットワークエラー (3回失敗) | ダウンロード失敗 | `in_progress → failed` | 最大リトライ回数到達 |
| ストレージ容量不足 | Stashapp レスポンス413 | `in_progress → failed` | エラー記録と管理者通知 |
| ツイートアーカイブ | `tweets.status = 'archived'` | `media.processing_state = 'archived'` | 関連メディアも論理アーカイブ |
| 管理者復元リクエスト | `/api/media/{id}/restore` | `archived → completed` | 管理者権限チェック必須 |

#### 4.3.2 ユーザー操作による遷移

| ユーザー操作 | 条件 | 遷移 |
|-------------|------|------|
| 「非表示にする」 | メディア選択時 | `processing_state` を `archived` に更新 |
| 「再試行する」 | `processing_state = 'failed'` | `failed → queued` (リトライ開始) |
| 「永久削除リクエスト」 | ユーザーからのリクエスト | **拒否** (永久保存ポリシーにより) |
| 「アーカイブ解除」 | 管理者権限あり | `archived → completed` |

#### 4.3.3 定期的な自動処理

| 処理内容 | 実行間隔 | 関連ステータス |
|----------|----------|----------------|
| ストレージ階層自動移行 | 毎日3時 | `completed` → `warm/cold` 移行判定 |
| 重複排除チェック | 毎時 | `queued/in_progress` のハッシュ比較 |
...
```sql
-- mediaテーブルのCHECK制約
CHECK (processing_state IN ('queued', 'in_progress', 'completed', 'failed', 'archived'))

-- 遷移不可能なパターンを防ぐトリガー
CREATE TRIGGER prevent_invalid_transition
BEFORE UPDATE ON media
FOR EACH ROW
WHEN (OLD.processing_state = 'completed' AND NEW.processing_state = 'queued')
THEN
    SELECT RAISE_APPLICATION_ERROR(-1, 'completedからqueuedへの遷移は不適切です');
```
...
本レポートで記載した通り、x_timeline_appプロジェクトでは**永久保存の原則**に基づき、メディアデータの物理的削除は一切実行されず、すべてのデータは論理的なステータスフラグ (`processing_state` / `status`) による管理のみで行われます。これにより、以下の利点が得られます。

1. **データの永続性**: ユーザーの意図に関わらず、すべてのメディアが永久に保持されます
2. **ストレージ最適化**: 階層ストレージ（Hot/Warm/Cold）による効率的な容量管理
3. **復旧柔軟性**: 管理者権限があればいつでも `archived` から `completed` への復元が可能
