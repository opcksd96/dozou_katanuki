...
def build_orig_image_url(url: str) -> str:
    """pbs.twimg.comの画像URLを最高画質 (orig) に変換"""
...
def extract_media_from_syndication(details: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for item in details or []:
        ...
        mp4_vars = [v for v in variants_raw if v.get("content_type") == "video/mp4" and "url" in v]
        mp4_vars = sorted(mp4_vars, key=lambda x: x.get("bitrate", x.get("bit_rate", 0)), reverse=True)
        variants = [{"url": v["url"], "bit_rate": v.get("bitrate", v.get("bit_rate", 0))} for v in mp4_vars]
...
def extract_media_from_html_card(card_soup: Any) -> List[Dict[str, Any]]:
    ...
    for img in card_soup.find_all("img"):
        ...
        if "pbs.twimg.com/media/" in src:
            orig = build_orig_image_url(src); fn = get_filename_from_url(orig)
            base_id = fn.split(".")[0]
            if base_id not in seen:
                ...
                media_list.append({
                    ...
                    "filename": fn, "thumbnail_url": src, "variants": []
                })
    for vid in card_soup.find_all("video"):
        ...
        if src and src not in seen:
            ...
            media_list.append({
                ...
                "width": 0, "height": 0, "bitrate": 0, "duration": 0.0,
                "filename": fn, "thumbnail_url": vid.get("poster", ""), "variants": []
            })
    return media_list
