...
class BaseScraper:
    """Wayback CDX走査 & WARCキャッシュ優先読み込み & 原本WARC保存基底エンジン"""
    ...
    def get_cdx_url_pattern(self, account: str) -> str:
        """プラットフォーム固有のCDX走査対象URLパターンを返却（具象クラスでオーバーライド）"""
        clean = account.lstrip("@").strip()
        return f"{self.platform}.com/{clean}"

    def _get_warc_path(self, original_url: str, timestamp: str, account: str = "") -> str:
        m = re.search(r'status(?:es)?/(\d+)', original_url) or re.search(r'/p/([^/?#]+)', original_url)
        post_id = m.group(1) if m else timestamp
        m_user = re.search(r'(?:twitter\.com|x\.com|instagram\.com|threads\.net)/([a-zA-Z0-9_.]+)', original_url)
    ...
    def search_cdx(self, account: str, limit: int = 0, log_fn: Optional[Callable[[str], None]] = None) -> List[Dict[str, str]]:
        def _log(m: str):
            if log_fn: log_fn(m)
            print(f"[CDX:{self.platform}] {m}", flush=True)

        target_pattern = self.get_cdx_url_pattern(account)
        params = {"url": target_pattern, "matchType": "prefix", "output": "json",
                  "fl": "timestamp,original,mimetype,statuscode,digest", "filter": "!statuscode:[45]..", "collapse": "urlkey"}
        if limit > 0: params["limit"] = str(limit)
        _log(f"CDX Query: target='{target_pattern}' (matchType=prefix), limit={limit or 'unlimited'}")
        ...
        for attempt in range(1, 4):
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    _log(f"CDX OK: Retrieved {len(raw_items)} snapshots for @{account}")
                    return raw_items
                elif resp.status_code in (429, 503):
                    w = attempt * 3; _log(f"Rate-limited (HTTP {resp.status_code}). Waiting {w}s..."); time.sleep(w)
        ...
        _log(f"CDX Failed: Could not retrieve snapshots for @{account} after 3 attempts.")
        return []
