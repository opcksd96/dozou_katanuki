...
class Restorer:
    """Layer 2 (生データダンプ/WARC) からマルチスレッド並列で高速オフライン DB 再構築エンジン"""
    def __init__(self, dumps_dir: str = "backups/dumps", db_path: str = "archive.db", storage_dir: Optional[str] = None, avatar_dir: str = "assets/avatars", max_workers: int = 8, parser: Optional[BaseParser] = None, platform: str = "base"):
        self.dumps_dir, self.db_path, self.avatar_dir, self.max_workers = dumps_dir, db_path, avatar_dir, max_workers
    ...
    def _sync_media_files(self) -> int:
        ...
        if updates:
            with self.downloader._get_conn() as conn:
                conn.cursor().executemany("UPDATE media SET download_status = ?, failed_reason = ?, stash_image_id = coalesce(?, stash_image_id), stash_scene_id = coalesce(?, stash_scene_id) WHERE media_id = ?", updates); conn.commit()
        return synced
    ...
    def _process_dir(self, pdir: str) -> Dict[str, int]:
        ...
        if os.path.exists(warc_path) and self.parser:
            try:
                with open(warc_path, "rb") as s:
                    for r in ArchiveIterator(s):
                        ...
                        if "json" in c_type or "html" in c_type or "status" in uri:
                            ...
                            if parsed and self.mutator.upsert_record(parsed): res["articles"] += 1
                        elif any(d in uri for d in ["twimg.com", "video.twimg.com", "cdn", "media"]):
                            mid = os.path.basename(uri.split("?")[0])
                            mt = "video" if any(mid.endswith(ext) for ext in [".mp4", ".webm", ".m3u8"]) else "image"
                            dest = self.downloader.get_target_path("unknown", mid, mt)
    ...
    def run_restore(self, progress_callback: Optional[Callable[[int, int, str], None]] = None) -> Dict[str, int]:
        ...
        os.makedirs(self.avatar_dir, exist_ok=True); stats = {"articles": 0, "media": 0, "avatars": 0}
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            ...
            for fut in concurrent.futures.as_completed(futures):
                completed += 1
                try: r = fut.result(); stats["articles"] += r["articles"]; stats["avatars"] += r["avatars"]
                except Exception: pass
