...
import base64, os, re, requests, sqlite3, time
...
class StashReconciler:
    ...
    def __init__(self, stash: Optional[StashClient] = None):
        self.stash, self.session = stash or StashClient(), requests.Session()

    def _fetch_thumbnail_b64(self, thumb_url: str) -> Optional[str]:
        if not thumb_url: return None
        try:
            r = self.session.get(thumb_url, timeout=4)
            if r.status_code == 200 and r.content and "image" in r.headers.get("Content-Type", "image/jpeg"):
                mime = r.headers.get("Content-Type", "image/jpeg")
                return f"data:{mime};base64,{base64.b64encode(r.content).decode('utf-8')}"
        except Exception: pass
        return None

    def register_media(self, file_path: str, media_type: str = "image", title: str = "", details: str = "", urls: Optional[List[str]] = None, date: str = "", username: str = "", display_name: str = "", thumbnail_url: str = "", custom_fields: Optional[Dict[str, Any]] = None, max_wait: float = 10.0) -> Optional[str]:
        ...
        if m_id:
            ...
            cov = self._fetch_thumbnail_b64(thumbnail_url) if media_type != "image" and thumbnail_url else None
            if media_type == "image":
                self.stash.update_image(m_id, title=title, details=details, urls=urls, date=date, studio_id=s_id, performer_ids=[p_id] if p_id else None, custom_fields=custom_fields)
            else:
                self.stash.update_scene(m_id, title=title, details=details, urls=urls, date=date, studio_id=s_id, performer_ids=[p_id] if p_id else None, cover_image=cov, custom_fields=custom_fields)
        return m_id

    def reconcile_to_db(self, db_path: str, log_fn: Optional[Callable[[str], None]] = None) -> int:
        def _log(msg: str) -> None: (log_fn and log_fn(msg)); print(f"[RECONCILE] {msg}", flush=True)
        ...
        scenes, images, bound = res.get("allScenes", []), res.get("allImages", []), 0
                ...
                for item in items:
                    ...
                    for f in files:
                        ...
                        if bn:
                            ...
                            row = cur.fetchone(); art_id = row[0] if row else art_id
    ...
    def _sync_parent_article(self, cur: sqlite3.Cursor, art_id: str, s_id: str, title: str, is_scn: bool) -> None:
        cur.execute("SELECT a.full_text, a.full_text_ja, a.created_at, a.wayback_url, ac.username, ac.display_name, m.thumbnail_url FROM articles a JOIN accounts ac ON a.account_id = ac.numeric_id LEFT JOIN media m ON m.article_id = a.id AND m.type = 'video' WHERE a.id = ?", (art_id,))
        row = cur.fetchone()
        if not row: return
        ft, ft_ja, cr_at, wb_url, uname, dname, thumb = row[0] or "", row[1] or "", str(row[2] or ""), row[3] or "", row[4] or "", row[5] or "", row[6] or ""
        txt, urls = (f"{ft_ja}\n\n{ft}".strip() if ft_ja and ft_ja != ft else (ft or ft_ja)), ([f"https://twitter.com/{uname}/status/{art_id}"] if uname and art_id else [])
        if wb_url: urls.append(wb_url)
        s_obj, p_obj = (self.stash.find_or_create_studio(uname) if uname else None), (self.stash.find_or_create_performer(uname, disambiguation=dname) if uname else None)
        ...
        cov = self._fetch_thumbnail_b64(thumb) if is_scn and thumb else None
        (self.stash.update_scene if is_scn else self.stash.update_image)(
            s_id, title=title, details=txt, urls=urls, date=cr_at[:10], studio_id=s_obj, performer_ids=[p_obj] if p_obj else None, cover_image=cov, custom_fields=cf
        ) if is_scn else (self.stash.update_image)(s_id, title=title, details=txt, urls=urls, date=cr_at[:10], studio_id=s_obj, performer_ids=[p_obj] if p_obj else None, custom_fields=cf)
