...
class BaseMutator:
    ...
    def _ensure_schema(self, conn: sqlite3.Connection) -> None:
        ...
        for col, ctype in [("thumbnail_url", "TEXT"), ("tweet_urls", "TEXT"), ("media_quality", "TEXT")]:
    ...
    def _prepare_in_memory(self, records: List[Dict[str, Any]]) -> Tuple[List[tuple], List[tuple], List[tuple], List[tuple], List[tuple], int]:
        now_ts, accounts, hists, redirects, articles, media, valid_count = time.strftime("%Y-%m-%d %H:%M:%S", time.gmtime()), [], [], [], [], [], 0
        for data in records:
            ...
            for u in post.get("urls", []):
                (u.get("short_url") and u.get("expanded_url") and redirects.append((u["short_url"], u["expanded_url"], post_id)) and (ftext := ftext.replace(u["short_url"], u["expanded_url"]) if u["short_url"] in ftext else ftext))
            ...
            for m in m_list:
                ...
                media.append((clean_mid, post_id, m.get("type", "image"), m_url, m.get("width", 0), m.get("height", 0), m.get("thumbnail_url"), t_urls_json, m.get("quality", "")))
            valid_count += 1
    ...
    def upsert_batch(self, records: List[Dict[str, Any]]) -> Tuple[int, int, int]:
        ...
        with self._get_conn() as conn:
            ...
            cur.executemany("""INSERT INTO media (media_id, article_id, type, download_url, width, height, thumbnail_url, tweet_urls, media_quality, download_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'QUEUED')
                ON CONFLICT(media_id) DO UPDATE SET download_url=case when (media.download_url IS NULL OR media.download_url = '' OR media.download_status = 'DEAD_404') and excluded.download_url != '' then excluded.download_url else media.download_url end, width=case when excluded.width > 0 then excluded.width else media.width end, height=case when excluded.height > 0 then excluded.height else media.height end, thumbnail_url=coalesce(excluded.thumbnail_url, media.thumbnail_url), tweet_urls=coalesce(excluded.tweet_urls, media.tweet_urls), media_quality=coalesce(excluded.media_quality, media_quality), download_status=case when media.download_status = 'DEAD_404' and excluded.download_url != '' then 'QUEUED' else media.download_status end""", meds)
            conn.commit()
