...
class BaseMutator:
    ...
    def _ensure_schema(self, conn: sqlite3.Connection) -> None:
        ...
        cur.execute("CREATE TABLE IF NOT EXISTS media_variants (variant_hash TEXT PRIMARY KEY, media_id TEXT, article_id TEXT, download_url TEXT, bit_rate INTEGER)"); conn.commit()
    ...
    def _prepare_in_memory(self, records: List[Dict[str, Any]], wl: set = None) -> Tuple[List[tuple], List[tuple], List[tuple], List[tuple], List[tuple], List[tuple], List[tuple], int]:
        ...
        for data in records:
            ...
            nit_u = post.get("nitter_url") or (f"https://{sdom}/{u_name}/status/{post_id}" if "nitter" in sname else None)
            tws_u = post.get("twistalker_url") or (f"https://twstalker.com/{u_name}/status/{post_id}" if ("twistalker" in sname or "twstalker" in sname) else None)
            ...
            t_urls_json, is_wl = json.dumps([u for u in [orig_url, sotwe_u, wb_u] if u], ensure_ascii=False), (not wl_set) or (u_name in wl_set)
            for m in m_list:
                ...
                if is_wl:
                    ...
                    for var in m.get("variants", []):
                        ...
                        media_variants.append((v_fn.split(".")[0], clean_mid, post_id, v_url, var.get("bit_rate", 0)))
                else: excluded.append((clean_mid, post_id, acc_id, m.get("type", "image"), m_url, m.get("width", 0), m.get("height", 0), "EXCLUDED", "Whitelist外", t_urls_json, m.get("thumbnail_url"), f"Non-whitelist: {u_name}"))
            valid_count += 1
    ...
    def upsert_record(self, data: Dict[str, Any]) -> bool: return (self.upsert_batch([data])[2] if isinstance(self.upsert_batch([data]), tuple) else self.upsert_batch([data])) == 1
