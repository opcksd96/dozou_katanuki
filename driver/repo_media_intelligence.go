...
func (r *Repository) MergeDuplicateMedia() (int, error) {
	...
	err := r.db.Model(&models.Media{}).
		Select("download_url, count(*) as count").Where("download_url != ''").
		Group("download_url").Having("count(*) > 1").Scan(&duplicates).Error
...
func (r *Repository) PurgeLowerResolutionDuplicates(trashDir string) (int, error) {
	...
	for _, group := range groups {
		...
		maxIdx, maxArea := 0, group[0].Width*group[0].Height
		for i, m := range group {
			if area := m.Width * m.Height; area > maxArea { maxArea, maxIdx = area, i }
		}

		for i, m := range group {
			if i == maxIdx { continue }
			if filePath, err := r.ResolveMediaFilePath(m.MediaID); err == nil && filePath != "" {
