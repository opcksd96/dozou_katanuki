<!-- frontend/src/views/admin/AdminConsole.vue (100行以下 - SPEC-ADMINBOARD-001) -->
...
import AdminMobileMenu from '../../components/admin/AdminMobileMenu.vue';
import PluginHubView from '../../components/admin/PluginHubView.vue';
import RelationExplorerView from '../../components/admin/RelationExplorerView.vue';
import ConfigPortal from '../../components/admin/ConfigPortal.vue';
import PipelineConsoleView from '../../components/admin/PipelineConsoleView.vue';
import AuditReportView from '../../components/admin/AuditReportView.vue';
import SystemConsoleView from '../../components/admin/SystemConsoleView.vue';
import DatabaseView from '../../components/admin/DatabaseView.vue';
...
onUnmounted(() => { window.removeEventListener('keydown', handleKey); if (unoffStash) try { unoffStash(); } catch {}; });
</script>

<template>
  <div class="w-screen h-screen max-h-[100dvh] bg-slate-950 flex flex-col overflow-hidden select-none font-sans">
    ...
    <div class="flex-1 min-h-0 flex overflow-hidden">
      ...
      <main class="flex-1 min-h-0 overflow-y-auto p-2 sm:p-3 bg-slate-950 flex flex-col">
        ...
        <PipelineConsoleView v-else-if="activeTab === 'pipeline'" class="w-full" @jump-to-media="() => { activeTab = 'media'; admin.fetchMedia?.(); }" />
