...
const { focusedIndex, isHelpOpen } = useKeyboardNavigation({
  ...
  isOverlayOpen: () => !activeMedia.value, isAdminOpen: () => false, openAdmin
});
...
onMounted(() => {
  ...
  if (sessionStorage.getItem('admin_auto_open_tab')) { openAdmin(); }
  try { if ((window as any)?.runtime?.EventsOnMultiple) (window as any).runtime.EventsOnMultiple('open:admin', () => { openAdmin(); }, -1); } catch {}
});
</script>
...
<template>
  <div class="min-h-screen bg-slate-950/85 text-slate-100 flex flex-col items-center selection:bg-blue-500/30 selection:text-blue-200">
    <GlobalAppBar :active-article-id="activeArticleId" :active-article-handle="detail?.article?.author.handle || ''" :is-stash-online="isStashReady" @open-admin="openAdmin" @back-to-timeline="closeDetail" />
