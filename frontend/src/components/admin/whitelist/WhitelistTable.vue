...
const editForm = reactive({ type: 'account', value: '', is_active: true });

const startEdit = (item: WhitelistItem) => {
  editingId.value = item.id;
  editForm.type = item.type; editForm.value = item.value; editForm.is_active = item.is_active;
};
    ...
    <table class="w-full text-left text-xs border-collapse">
      <thead>
        <tr class="border-b border-slate-800 bg-slate-900/80 text-slate-400 font-mono">
          <th class="py-2.5 px-4 w-14">ID</th><th class="py-2.5 px-4 w-28">種別</th>
          <th class="py-2.5 px-4">ルール値</th><th class="py-2.5 px-4 w-28 text-center">稼働状態</th>
          <th class="py-2.5 px-4 w-36 text-right">アクション</th>
        </tr>
      </thead>
      <tbody class="divide-y divide-slate-800/60">
        <tr v-if="items.length === 0">
          <td colspan="5" class="py-8 text-center text-slate-500">{{ loading ? '読込中...' : '該当項目なし' }}</td>
        </tr>
