...
<template>
  <div v-show="isOpen" class="fixed inset-0 z-50 w-screen h-screen bg-slate-950 flex flex-col overflow-hidden select-none overscroll-contain">
    <!-- モーダルヘッダー (薄型コンパクト) -->
    <div class="px-4 py-1.5 border-b border-slate-800 flex items-center justify-between bg-slate-900/90 shrink-0">
      <div class="flex items-center gap-2">
        <span class="text-base">⚙️</span>
        <h2 class="text-xs font-bold text-slate-200">土蔵・型抜き Admin Governance Portal</h2>
      </div>
      <button @click="$emit('close')" class="px-2.5 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white flex items-center gap-1 text-xs font-bold transition-colors border border-slate-700 shadow">
        <span>←</span> タイムラインに戻る
      </button>
    </div>

    <!-- タブバー (薄型コンパクト) -->
    <div class="px-4 bg-slate-900/40 border-b border-slate-800 flex gap-1.5 overflow-x-auto text-xs font-semibold shrink-0">
      <button v-for="t in [
      ...
      ]" :key="t.id" @click="activeTab = t.id as any" class="py-1.5 px-2.5 border-b-2 transition-colors whitespace-nowrap" :class="activeTab === t.id ? 'border-blue-500 text-blue-400 bg-blue-500/10' : 'border-transparent text-slate-400 hover:text-slate-200'">
        {{ t.label }}
      </button>
    </div>

    <!-- コンテンツ領域（画面いっぱいに広がるメインワークスペース） -->
    <div class="flex-1 min-h-0 overflow-hidden p-2.5 bg-slate-950 flex flex-col">
      ...
      <ConfigPortal v-else-if="activeTab === 'config'" class="overflow-y-auto flex-1" :config="admin.configForm" :loading-config="admin.isConfigLoading.value" :saving-config="admin.isConfigLoading.value" :save-status="admin.configSaved.value ? { success: true, message: '設定を保存しました' } : null" @save-config="admin.saveConfig" @load-config="admin.fetchConfig" />
      <SkinFontEditor v-else-if="activeTab === 'skin'" class="overflow-y-auto flex-1" :skin-c-s-s="admin.skinCSS.value" :loading-skin="admin.isSkinLoading.value" :saving-skin="admin.isSkinLoading.value" :skin-status="admin.isSkinSaved.value ? { success: true, message: 'スキンを保存しました' } : null" :selected-platform="selectedPlatform" :font-presets="fontPresets" :config="admin.configForm" :saving-config="admin.isConfigLoading.value" :save-status="null" @fetch-skin="(p) => admin.fetchSkinCSS(p)" @save-skin="(p, css) => admin.saveSkinCSS(p, css)" @apply-dynamic-skin="() => {}" @save-config="admin.saveConfig" />
      <WhitelistView v-else-if="activeTab === 'whitelist'" class="overflow-y-auto flex-1" :whitelist-list="admin.whitelists.value" :loading="admin.isWhitelistLoading.value" :status-message="null" @fetch="admin.fetchWhitelists" @add="async (t, v) => { await admin.addWhitelist(t, v); onWhitelistChanged(); }" @update="async (id, t, v, act) => { await admin.updateWhitelist(id, t, v, act); onWhitelistChanged(); }" @delete="async (id) => { await admin.deleteWhitelist(id); onWhitelistChanged(); }" @toggle="async (id) => { await admin.toggleWhitelist(id); onWhitelistChanged(); }" />
      <DatabaseView v-else-if="activeTab === 'db'" class="flex-1 min-h-0" :admin="admin" />
      <AuditReportView v-else-if="activeTab === 'audit'" class="overflow-y-auto flex-1" :report="admin.auditReport.value" :loading="admin.isAuditing.value" :purging-files="admin.isPurgingFiles.value" :purging-db="admin.isPurgingDB.value" :status-message="admin.auditStatusMessage.value" :restoring="admin.isJobRunning.value" @run-audit="(pf, pdb) => admin.runAudit(pf, pdb)" @purge-orphan-files="(paths) => admin.purgeOrphanFiles(paths)" @purge-orphan-db-media="(ids) => admin.purgeOrphanDBMedia(ids)" @purgeOrphanDBMedia="(ids) => admin.purgeOrphanDBMedia(ids)" @trigger-restore="(resetDB) => { admin.triggerRestore('', resetDB); activeTab = 'jobs'; }" />
