<!-- frontend/src/components/admin/DownloaderConsoleView.vue (100行以下) -->
...
<template>
  <div class="h-full flex flex-col p-4 space-y-4 bg-slate-950 text-slate-100 overflow-y-auto font-sans">
    <!-- ヘッダー -->
    <div class="flex items-center justify-between border-b border-slate-800 pb-3">
      <div class="flex items-center gap-2">
        <span class="text-xl">⚡</span>
        <h2 class="text-sm font-bold tracking-wide">Motrix Next ＆ Thunder 管理コンソール</h2>
      </div>
      <button @click="fetchStatus" class="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded text-xs active:scale-95 transition-transform cursor-pointer">🔄 更新</button>
    </div>

    <!-- 上段: 2大ダウンローダーサマリー -->
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      <!-- Motrix Next (Aria2) パネル -->
      <div class="p-4 bg-slate-900/90 border border-slate-800 rounded-2xl space-y-3 shadow-lg">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-2">
            <span class="text-base">🚀</span>
            <span class="font-bold text-xs">Motrix Next (Aria2 Engine)</span>
          </div>
        ...
        <div class="grid grid-cols-4 gap-2 text-center">
          ...
          <div class="p-2 bg-slate-950 rounded-xl border border-slate-800"><div class="text-[10px] text-slate-500">待機キュー</div><div class="text-sm font-mono font-bold text-amber-400">{{ status?.motrix?.num_waiting ?? 0 }}</div></div>
          <div class="p-2 bg-slate-950 rounded-xl border border-slate-800"><div class="text-[10px] text-slate-500">完了/停止</div><div class="text-sm font-mono font-bold text-emerald-400">{{ status?.motrix?.num_stopped ?? 0 }}</div></div>
          <div class="p-2 bg-slate-950 rounded-xl border border-slate-800"><div class="text-[10px] text-slate-500">DL速度</div><div class="text-sm font-mono font-bold text-purple-400">{{ formatSpeed(status?.motrix?.download_speed) }}</div></div>
        </div>
      ...
      <div class="p-4 bg-slate-900/90 border border-slate-800 rounded-2xl space-y-3 shadow-lg flex flex-col justify-between">
        ...
        <div class="flex gap-2 pt-2">
          <button @click="launchThunder" :disabled="!status?.thunder?.is_installed" class="flex-1 py-1.5 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-white rounded-lg text-xs font-bold shadow-md active:scale-95 cursor-pointer disabled:opacity-50">🚀 Thunder.exe 起動</button>
      ...
      <div class="flex-1 overflow-y-auto space-y-2 pr-1">
        ...
        <div v-for="t in status?.motrix?.active_tasks" :key="t.gid" class="p-2.5 bg-slate-950 rounded-xl border border-slate-800/80 space-y-1.5">
          ...
          <div class="w-full bg-slate-900 rounded-full h-1.5 overflow-hidden">
            <div class="bg-gradient-to-r from-blue-500 to-purple-500 h-full rounded-full transition-all duration-300" :style="{ width: `${t.progress}%` }"></div>
          </div>
