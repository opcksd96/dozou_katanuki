    ...
    <table class="w-full text-left text-xs font-mono">
      <thead class="bg-slate-900 text-slate-400 border-b border-slate-800 text-[11px]">
        <tr>
          ...
          <th class="p-2.5 w-16 text-center">プレビュー</th>
          <th class="p-2.5">タイトル / 記事詳細</th>
          <th class="p-2.5">アカウント</th>
          <th class="p-2.5">種別</th>
          <th class="p-2.5">解像度</th>
          <th class="p-2.5">ステータス</th>
          <th class="p-2.5">Stash ID</th>
          <th class="p-2.5 text-right">アクション</th>
        </tr>
      </thead>
      <tbody class="divide-y divide-slate-850">
        ...
        >
          ...
          <td class="p-2.5 max-w-[280px]">
            <div class="font-bold text-slate-200 truncate text-xs" :title="m.title || m.media_id">
              {{ m.title || `X (@${m.username}): Tweet ${m.article_id}` }}
            </div>
          ...
          <td class="p-2.5 text-slate-300">
            <div class="flex items-center gap-1.5">
              <Avatar :avatar-url="m.avatar_url" :handle="m.username" size-class="w-4 h-4" />
          ...
          <td class="p-2.5 text-[11px] text-purple-400 font-bold">
            {{ m.stash_scene_id || m.stash_image_id ? (m.stash_scene_id || m.stash_image_id).slice(0, 8) : '-' }}
          </td>
