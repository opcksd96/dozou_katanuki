...
<template>
  <div class="p-2.5 bg-slate-900 border border-slate-800 rounded-xl flex flex-nowrap overflow-x-auto no-scrollbar items-center justify-between gap-2 text-xs">
    <!-- フィルタ・検索群 -->
    <div class="flex items-center gap-2 shrink-0">
      <input :value="searchQuery" @input="emit('update:searchQuery', ($event.target as HTMLInputElement).value)" placeholder="🔍 検索..." class="w-24 md:w-40 bg-slate-950 border border-slate-700 rounded px-2 py-1 text-slate-200" />
      ...
      <div class="flex rounded border border-slate-700 overflow-hidden text-[11px] shrink-0">
      ...
      <button @click="emit('update:onlyBookmarked', !onlyBookmarked)" :class="onlyBookmarked ? 'bg-amber-600 text-white' : 'bg-slate-950 text-slate-400 border border-slate-700'" class="px-2 py-1 rounded text-[11px] cursor-pointer transition-colors active:scale-95 shrink-0 whitespace-nowrap">
        ★<span class="hidden md:inline"> ブックマーク</span>
      </button>
    </div>
    ...
    <div class="flex items-center gap-2 shrink-0">
      ...
      <button @click="emit('reconcileStash')" :disabled="isReconcileRunning" :class="isReconcileRunning ? 'opacity-80 ring-2 ring-purple-400 animate-pulse' : 'hover:bg-purple-500 active:scale-95 shadow-sm'" class="px-2.5 py-1.5 bg-purple-600 text-white rounded-lg text-[11px] font-bold flex items-center gap-1 transition-all duration-150 cursor-pointer disabled:cursor-not-allowed whitespace-nowrap" title="Stash照合: ローカル保存ファイルをStashと照合してCOMPLETEDへ昇格">
        ...
        <span class="hidden xl:inline">{{ isReconcileRunning ? '照合中...' : 'Stash照合' }}</span>
      </button>
      <button @click="emit('openStash')" class="px-2 py-1.5 bg-purple-950/90 hover:bg-purple-900 border border-purple-500/70 text-purple-200 hover:text-white font-bold rounded-lg text-[11px] shadow-sm transition-all duration-150 active:scale-95 cursor-pointer flex items-center gap-1 whitespace-nowrap" title="Stash WebUIをブラウザで開く">
        <span>🎛️</span><span class="hidden xl:inline"> WebUI</span><span class="text-purple-400 hidden xl:inline">↗</span>
      </button>
    </div>
  </div>
</template>
