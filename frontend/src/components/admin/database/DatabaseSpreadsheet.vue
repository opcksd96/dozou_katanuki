...
<template>
  <div class="border border-slate-700/80 rounded-xl overflow-hidden bg-slate-950 flex flex-col h-full font-mono text-xs select-text shadow-inner">
    <div class="overflow-x-auto overflow-y-auto flex-1 max-h-[460px]">
      <table class="w-full border-collapse text-left">
        ...
        <tbody class="divide-y divide-slate-800/80 text-slate-300">
          ...
          <tr v-for="(r, idx) in rows" :key="r[idKey || 'id'] || idx" @click="emit('selectRow', r)" class="hover:bg-blue-900/20 cursor-pointer transition-colors" :class="{ 'bg-blue-950/40 text-white font-medium': selectedRowId && (r[idKey || 'id'] === selectedRowId) }">
