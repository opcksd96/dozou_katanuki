...
const cols = [
  { key: 'is_whitelist', label: '🛡️ 巡回', width: '75px' },
  { key: 'numeric_id', label: 'ID', width: '100px' }, { key: 'username', label: 'Handle', width: '105px' },
  { key: 'display_name', label: 'Name', width: '120px' },
];
...
<template>
  <div class="flex-1 flex flex-col md:flex-row min-h-0 bg-slate-950 overflow-hidden font-sans">
    <!-- 左側: アカウント一覧スプレッドシート (Whitelist トグル付き) -->
    <div class="w-full md:w-1/2 flex flex-col border-r border-slate-800 min-h-0">
      <div class="p-3 bg-slate-900 border-b border-slate-800 flex items-center justify-between">
        <h3 class="text-xs font-bold uppercase tracking-wider text-slate-300">👥 アカウント・巡回一覧 ({{ accounts?.length || 0 }}件)</h3>
      ...
      <div class="flex-1 min-h-0 overflow-y-auto">
        <DatabaseSpreadsheet
          :columns="cols" :rows="accounts" :selected-row-id="selectedDetail?.account?.numeric_id" :id-key="'numeric_id'"
          @select-row="emit('selectAccount', $event?.numeric_id || $event)"
        >
          <template #cell-is_whitelist="{ row }">
            <button
              @click.stop="emit('toggleWhitelist', row.numeric_id, !row.is_whitelist)"
              class="px-2 py-0.5 rounded text-[10px] font-bold font-mono transition-all cursor-pointer shadow-sm"
              :class="row.is_whitelist ? 'bg-emerald-950 text-emerald-300 border border-emerald-700/60' : 'bg-slate-800 text-slate-400 hover:text-slate-200'"
              :title="row.is_whitelist ? '巡回・保存対象 (ON)' : '外部参照のみ (OFF)'"
            >
    ...
    <!-- 右側: アカウント詳細・名寄せ・歴代変遷史 -->
    <div class="w-full md:w-1/2 flex flex-col min-h-0 overflow-y-auto p-4 space-y-4 bg-slate-950">
      ...
      <template v-else-if="selectedDetail?.account">
        <AccountDetailCard
          :account="selectedDetail.account" :post-count="selectedDetail.post_count || 0"
          :is-editing="isEditing" :available-avatars="availableAvatars"
          @start-edit="isEditing = true" @cancel-edit="isEditing = false"
          @save="handleSave" @toggle-whitelist="emit('toggleWhitelist', selectedDetail.account.numeric_id, !selectedDetail.account.is_whitelist)"
          @view-posts="emit('viewPosts', selectedDetail.account.numeric_id)" @view-media="emit('viewMedia', selectedDetail.account.numeric_id)"
        />
