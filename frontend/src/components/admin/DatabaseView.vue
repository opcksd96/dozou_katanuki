...
const emit = defineEmits<{
  (e: 'navigate', tab: 'accounts' | 'posts' | 'media'): void;
...
const handleToggleWhitelist = async (numericId: string, isWhitelist: boolean) => {
  if (await props.admin?.toggleAccountWhitelist?.(numericId, isWhitelist)) {
    addToast(`🛡️ 巡回ステータスを [${isWhitelist ? 'ON (巡回対象)' : 'OFF (外部記録)'}] に変更しました`, 'info', 3000);
  }
};
...
const onNavigatePost = async (id: string) => {
  ...
  await props.admin?.searchArticles?.(1); emit('navigate', 'posts');
};

const handleSmartRecovery = async () => { addToast('⚡ スマート一括回収ジョブを開始しました（直接 ➔ Motrix ➔ Stash）', 'info', 4000); await props.admin?.startSmartRecovery?.(); };
const handleThunderEscalate = async () => { addToast('⚡ 迅雷 (Thunder.exe) へメディア直リンクを投入しました', 'info', 4000); await props.admin?.startThunderEscalate?.(); };
const handleReconcileStash = async () => {
  addToast('📦 Stash との照合同期を開始しました...', 'info', 3000);
  const count = await props.admin?.reconcileStashMedia?.(); addToast(`✅ Stash同期完了: ${count || 0} 件を照合・バインドしました`, 'success', 3000);
};
</script>
