...
const handleMergeAccounts = async (sourceId: string, targetId: string) => {
  if (await props.admin?.mergeAccounts?.(sourceId, targetId)) {
    ...
    addToast(`❌ アカウントの合併に失敗しました`, 'error', 3000);
  }
};
