...
const emit = defineEmits<{ (e: 'navigate', view: 'accounts' | 'posts' | 'media'): void; (e: 'jumpToTimelinePost', articleId: string): void }>();

const { addToast } = useToast();
const localSelected = ref<any>(null), availableAvatars = ref<string[]>([]);

const loadAvatars = async () => {
  try { availableAvatars.value = (await props.admin?.fetchAvailableAvatars?.('twitter')) || []; } catch {}
};
onMounted(() => loadAvatars());

const handleAutoTranslate = async (autoSave: boolean) => {
