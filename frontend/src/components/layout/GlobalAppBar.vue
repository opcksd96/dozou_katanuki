...
import { RefreshCw, Settings, Minus, Square, X, Server, Layers, Menu } from 'lucide-vue-next';
...
<template>
  <nav class="w-full bg-slate-950/70 backdrop-blur-xl border-b border-white/10 sticky top-0 z-40 px-3 py-2 flex items-center justify-between select-none wails-drag transition-colors">
    ...
    <div class="flex items-center gap-1.5">
      ...
      <!-- デスクトップ表示時: Stash/再読込/設定・管理 ボタン群 -->
      <div class="hidden sm:flex items-center gap-1.5 wails-no-drag">
        <div class="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-slate-900/80 border border-slate-800/80 text-[11px] font-mono">
          <span :class="['w-1.5 h-1.5 rounded-full', isOnline ? 'bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.8)]' : 'bg-amber-500']"></span>
          <span :class="isOnline ? 'text-slate-300' : 'text-slate-500'" class="flex items-center gap-1"><Server class="w-3 h-3 text-slate-400" />Stash</span>
        </div>
