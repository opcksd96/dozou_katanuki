...
// SchedulerService は定期ポーリングおよび定期バックアップを常駐管理する軽量スケジューラーです (SPEC-SCHEDULER-001)
...
func (s *SchedulerService) Start(ctx context.Context) {
	...
	log.Printf("[Scheduler] Started: PollInterval=%ds, BackupInterval=%dh, MaxGenerations=%d",
		s.cfg.PollIntervalSec, s.cfg.BackupIntervalHours, s.cfg.MaxBackupGenerations)
...
func (s *SchedulerService) TriggerPoll() (*models.JobProgress, error) {
	log.Println("[Scheduler] Triggering manual media poll (Phase 3 Stash recovery)...")
...
func (s *SchedulerService) TriggerBackup() (string, error) {
	log.Println("[Scheduler] Triggering manual Layer 1 online backup (VACUUM INTO)...")
	if s.repo == nil {
		return "", nil
	}
	path, err := s.repo.BackupDatabase("backups/database", s.cfg.MaxBackupGenerations)
	if err != nil {
		log.Printf("[Scheduler] Backup error: %v", err)
		if s.emitter != nil {
			s.emitter("scheduler:backup_failed", map[string]string{"error": err.Error()})
		}
		return "", err
	}
	log.Printf("[Scheduler] Backup completed successfully: %s", path)
	if s.emitter != nil {
		s.emitter("scheduler:backup_completed", map[string]string{"path": path})
	}
	return path, nil
}
