...
func (a *App) syncArticleDetailsToStash(stashID, articleID string, isScene bool) {
	...
	if sIDObj := a.findOrCreateStudio(uname); sIDObj != "" {
		input["studio_id"] = sIDObj
	}
	if pIDObj := a.findOrCreatePerformer(uname, dname); pIDObj != "" {
		input["performer_ids"] = []string{pIDObj}
	}

	mut := `mutation ImageUpdate($input: ImageUpdateInput!) { imageUpdate(input: $input) { id } }`
	if isScene {
		mut = `mutation SceneUpdate($input: SceneUpdateInput!) { sceneUpdate(input: $input) { id } }`
	}
