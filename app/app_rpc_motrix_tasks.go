// app/app_rpc_motrix_tasks.go (100行以下 - SPEC-PRINCIPLE-001)
package app
...
func (a *App) fetchMotrixActiveTasks() []models.DownloaderTaskInfo {
	...
	for _, t := range res.Result {
		...
		tasks = append(tasks, models.DownloaderTaskInfo{
			GID: t.GID, Status: t.Status, FileName: fn, URL: uri,
			TotalLength: tl, CompletedLength: cl, DownloadSpeed: ds,
			Progress: prog, ErrorMessage: t.ErrorMessage,
		})
	}
	return tasks
}
