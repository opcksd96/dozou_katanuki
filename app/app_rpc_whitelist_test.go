...
func TestWhitelistCRUD(t *testing.T) {
	...
	if err := repo.UpdateWhitelist(item.ID, "keyword", "retro_famicom", "Retro Gaming", "", true); err != nil { t.Fatalf("UpdateWhitelist failed: %v", err) }
	list3, _ := repo.GetWhitelists()
	if list3[0].Type != "keyword" || list3[0].Value != "retro_famicom" || list3[0].GroupName != "Retro Gaming" { t.Fatalf("UpdateWhitelist failed: %+v", list3[0]) }
