...
CREATE TABLE IF NOT EXISTS `account_profile_histories` (`id` integer PRIMARY KEY AUTOINCREMENT,`account_id` text NOT NULL,`display_name` text NOT NULL,`description` text DEFAULT '',`avatar_original_url` text NOT NULL,`avatar_seq` integer NOT NULL,`avatar_virtual_key` text NOT NULL,`avatar_base64` text,`observed_at` datetime NOT NULL,CONSTRAINT `fk_accounts_profile_history` FOREIGN KEY (`account_id`) REFERENCES `accounts`(`numeric_id`));
CREATE INDEX IF NOT EXISTS `idx_history_lookup` ON `account_profile_histories`(`account_id`, `avatar_seq` DESC);
CREATE TABLE IF NOT EXISTS `articles` (`id` text,`account_id` text NOT NULL,`conversation_id` text NOT NULL,`reply_to_id` text,`reply_to_handle` text,`created_at` datetime NOT NULL,`full_text` text NOT NULL,`lang` text NOT NULL DEFAULT "ja",`full_text_ja` text,`full_text_en` text,`full_text_zh` text,`via` text NOT NULL,`is_repost` boolean NOT NULL DEFAULT false,`is_liked` boolean NOT NULL DEFAULT false,`wayback_url` text NOT NULL,`source_name` text,`source_domain` text,`original_url` text,`sotwe_url` text,`nitter_url` text,`twistalker_url` text,`is_trash` boolean NOT NULL DEFAULT false,`trashed_by` text,`trash_reason` text,`trashed_at` datetime,PRIMARY KEY (`id`),CONSTRAINT `fk_accounts_articles` FOREIGN KEY (`account_id`) REFERENCES `accounts`(`numeric_id`));
